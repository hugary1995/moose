...
#include "ADComputeIsotropicMultipleInelasticStress.h"
#include "ADStressIsotropicUpdateBase.h"
#include "MooseException.h"

registerMooseObject("TensorMechanicsApp", ADComputeIsotropicMultipleInelasticStress);

InputParameters
ADComputeIsotropicMultipleInelasticStress::validParams()
{
  InputParameters params = ADComputeIsotropicFiniteStrainElasticStress::validParams();
...
ADComputeIsotropicMultipleInelasticStress::ADComputeIsotropicMultipleInelasticStress(
    const InputParameters & parameters)
  : ADComputeIsotropicFiniteStrainElasticStress(parameters),
...
void
ADComputeIsotropicMultipleInelasticStress::initQpStatefulProperties()
{
...
void
ADComputeIsotropicMultipleInelasticStress::initialSetup()
{
  ...
  {
    ADStressIsotropicUpdateBase * rrr =
        dynamic_cast<ADStressIsotropicUpdateBase *>(&this->getMaterialByName(models[i]));
    ...
    if (rrr)
    {
      ...
      mooseError("Model " + models[i] +
                 " is not compatible with ADComputeIsotropicMultipleInelasticStress");
  }
}

void
ADComputeIsotropicMultipleInelasticStress::computeQpStress()
{
...
void
ADComputeIsotropicMultipleInelasticStress::computeQpStressIntermediateConfiguration()
{
...
void
ADComputeIsotropicMultipleInelasticStress::finiteStrainRotation()
{
  _elastic_strain[_qp] =
...
void
ADComputeIsotropicMultipleInelasticStress::updateQpState(
    ADRankTwoTensor & elastic_strain_increment,
    ADRankTwoTensor & combined_inelastic_strain_increment)
{
  ...
  {
    _console << std::endl
             << "iteration output for ADComputeIsotropicMultipleInelasticStress solve:"
  ...
  if (counter == _max_iterations && l2norm_delta_stress > _absolute_tolerance &&
    ...
    mooseException(
        "In ",
        _name,
        ": Max stress iteration hit during ADComputeIsotropicMultipleInelasticStress solve!");
...
void
ADComputeIsotropicMultipleInelasticStress::updateQpStateSingleModel(
...
void
ADComputeIsotropicMultipleInelasticStress::computeAdmissibleState(
    unsigned model_number,
    ADRankTwoTensor & elastic_strain_increment,

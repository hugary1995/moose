...
registerMooseObject("MooseApp", NEML2SmallStrain);
...
NEML2SmallStrain::NEML2SmallStrain(const InputParameters & parameters) : NEML2PreKernel(parameters)
{
  ...
  _grad_disp_x = &_fem.getGradient(disp_vars[0]);
  _grad_disp_y = disp_vars.size() >= 2 ? &_fem.getGradient(disp_vars[1]) : nullptr;
  _grad_disp_z = disp_vars.size() >= 3 ? &_fem.getGradient(disp_vars[2]) : nullptr;
}

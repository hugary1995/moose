...
registerMooseObject("MooseApp", NEML2StressDivergence);
    ...
    _ndisp(_disp_vars.size())
{
  ...
  // Request ghosted residual
  // _sys.residualGhosted();

  if (_disp_vars.size() < 1 || _disp_vars.size() > 3)
    mooseError("NEML2StressDivergence requires 1 to 3 displacement variables, got ",
               _disp_vars.size());

  _grad_test_x = &_fem.getPhiGradient(_disp_vars[0]);
  _grad_test_y = _ndisp >= 2 ? &_fem.getPhiGradient(_disp_vars[1]) : nullptr;
  _grad_test_z = _ndisp >= 3 ? &_fem.getPhiGradient(_disp_vars[2]) : nullptr;
}
...
NEML2StressDivergence::forward()
{
  ...
  for (auto i : make_range(_ndisp))
  {
    ...
    const auto & dofmap_i = _fem.getGlobalDofMap(_disp_vars[i]);

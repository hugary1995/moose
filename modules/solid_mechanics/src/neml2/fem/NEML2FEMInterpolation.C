...
// MOOSE includes
#include "NEML2FEMInterpolation.h"
#include "Executioner.h"

using namespace libMesh;

registerMooseObject("MooseApp", NEML2FEMInterpolation);

InputParameters
NEML2FEMInterpolation::validParams()
{
...
NEML2FEMInterpolation::NEML2FEMInterpolation(const InputParameters & parameters)
  : ElementUserObject(parameters), _neml2_assembly(getUserObject<NEML2Assembly>("assembly"))
{
}

const neml2::Tensor &
NEML2FEMInterpolation::getValue(const std::string & var_name) const
{
...
const neml2::Tensor &
NEML2FEMInterpolation::getGradient(const std::string & var_name) const
{
...
const neml2::Tensor &
NEML2FEMInterpolation::getPhi(const std::string & var_name) const
{
  const auto * var = getMOOSEVariable(var_name);
...
const neml2::Tensor &
NEML2FEMInterpolation::getPhiGradient(const std::string & var_name) const
{
  const auto * var = getMOOSEVariable(var_name);
...
const neml2::Tensor &
NEML2FEMInterpolation::getDofMap(const std::string & var_name) const
{
  return _neml2_dof_map[var_name];
}

const std::vector<dof_id_type> &
NEML2FEMInterpolation::getGlobalDofMap(const std::string & var_name) const
{
  return _moose_dof_map_global.at(var_name);
}

int64_t
NEML2FEMInterpolation::local_ndof() const
{
  return _local_ndof;
}

const MooseVariableFE<Real> *
NEML2FEMInterpolation::getMOOSEVariable(const std::string & var_name) const
{
  ...
  if (!var_fe)
    mooseError("NEML2FEMInterpolation only supports variables of type MooseVariableFE<Real>");
...
void
NEML2FEMInterpolation::initialSetup()
{
...
void
NEML2FEMInterpolation::invalidateFEMContext() const
{
...
void
NEML2FEMInterpolation::invalidateInterpolations() const
{
  _interp_up_to_date = false;
}

void
NEML2FEMInterpolation::meshChanged()
{
  invalidateFEMContext();
}

void
NEML2FEMInterpolation::initialize()
{
...
void
NEML2FEMInterpolation::syncWithMainThread()
{
  auto & main_uo = _fe_problem.getUserObject<NEML2FEMInterpolation>(name(), /*tid=*/0);
...
void
NEML2FEMInterpolation::threadJoin(const UserObject & y)
{
  const auto & other = static_cast<const NEML2FEMInterpolation &>(y);
  mooseAssert(_fem_context_up_to_date == other._fem_context_up_to_date,
              "NEML2FEMInterpolation becomes out of sync with other thread");
...
void
NEML2FEMInterpolation::finalize()
{
...
void
NEML2FEMInterpolation::updateFEMContext()
{
...
void
NEML2FEMInterpolation::updateDofMap()
{
...
void
NEML2FEMInterpolation::updatePhi()
{
  auto device = _app.getLibtorchDevice();
  auto nelem = _neml2_assembly.numElem();
...
void
NEML2FEMInterpolation::updateGradPhi()
{
  auto device = _app.getLibtorchDevice();
  auto nelem = _neml2_assembly.numElem();
  auto nqp = _neml2_assembly.numQP();
...
void
NEML2FEMInterpolation::updateInterpolations()
{
  ...
  for (auto & [var_name, val] : _vars)
  {
    ...
    auto sol_scattered = neml2::fem_scatter(sol, dof_map);
    val = neml2::fem_interpolate(sol_scattered, phi);
  }
  ...
  for (auto & [var_name, val] : _grad_vars)
  {
    ...
    auto sol_scattered = neml2::fem_scatter(sol, dof_map);
    val = neml2::fem_interpolate(sol_scattered, grad_phi);
  }

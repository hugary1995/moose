    ...
    _prop(declareGenericProperty<T, is_ad>(_base_name + _prop_name))
{
  ...
  {
    mooseAssert(_keys[i].size() == _nvars,
                "The length of each base_name_key must be equal to the number of level set "
                "variables which is "
                    << _nvars);
...
LevelSetMultiMaterialTempl<T, is_ad>::computeProperties()
{
  ...
  for (unsigned int i = 0; i < _nvars; i++)
  {
    ...
    if ((_xfem->isPointInsidePhysicalDomain(_current_elem, *node)) == (ls_node_value > 0.0))
      key += std::pow(2, i);
  }

  // We may run into situations where the key doesn't exist in the base_name_map. This may happen
  // when the problem is not well defined, or the level-set variables are very nonlinear. Cutting
  // the timestep may help, so we throw an exception in this case.
  try
  {
  ...
  {
    throw MooseException("Unknown combination of signs of the level-set variables which is not "

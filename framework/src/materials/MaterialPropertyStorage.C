...
void
MaterialPropertyStorage::merge(MaterialData & material_data,
                               const Elem & elem_to,
                               const Elem & elem_from,
                               unsigned int side,
                               unsigned int n_qpoints,
                               Real weight)
{
  ...
  for (unsigned int i = 0; i < _stateful_prop_id_to_prop_id.size(); ++i)
  {
    for (unsigned int qp = 0; qp < n_qpoints; ++qp)
    {
      props(&elem_to, side)[i]->qpMerge(qp, props(&elem_from, side)[i], qp, weight);
      propsOld(&elem_to, side)[i]->qpMerge(qp, propsOld(&elem_from, side)[i], qp, weight);
      if (hasOlderProperties())
        propsOlder(&elem_to, side)[i]->qpMerge(qp, propsOlder(&elem_from, side)[i], qp, weight);
    }
  }
}

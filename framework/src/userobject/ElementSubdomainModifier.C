...
ElementSubdomainModifier::setMovingBoundaryName(MooseMesh & mesh)
{
  ...
  std::cout << "I am definitely creating the sideset and the nodeset" << std::endl;
}
                                             ...
                                             const std::vector<const Elem *> & moved_elems)
{
  ...
  for (auto elem : moved_elems)
  {
    for (auto side : elem->side_index_range())
    {
      ...
      if (neighbor)
      {
        // If the neighbor is not in the target subdomain, then this side should be added to
        // the moving boundary
...
void
ElementSubdomainModifier::setSolutionForMovedNodes(SystemBase & sys)
{

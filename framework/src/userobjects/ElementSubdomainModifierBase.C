...
ElementSubdomainModifierBase::validParams()
{
  ...
  params.addParam<bool>("original_subdomain_in_list",
                        true,
                        "If set to false, only elements which change from subdomains not listed in "
                        "'subdomains_to_reinitialize', to ones that are listed, are reinitialized. "
                        "This must be set with a non-empty list in 'subdomains_to_reinitialize'.");
  return params;
}
...
ElementSubdomainModifierBase::ElementSubdomainModifierBase(const InputParameters & parameters)
  : ElementUserObject(parameters),
    ...
    _original_subdomain_in_list(getParam<bool>("original_subdomain_in_list"))
{
}
...
ElementSubdomainModifierBase::initialSetup()
{
  const std::vector<SubdomainName> subdomain_names_to_reinitialize =
      getParam<std::vector<SubdomainName>>("subdomains_to_reinitialize");
  ...
  if (_original_subdomain_in_list == false &&
    ...
    paramError(
        "original_subdomain_in_list",
        "'original_subdomain_in_list' can only be set to false if subdomains_to_reinitialize does "
  ...
  else if (_original_subdomain_in_list == false && _subdomain_ids_to_reinitialize.empty())
    paramError("original_subdomain_in_list",
               "'original_subdomain_in_list' can only be set to false if "
               "subdomains_to_reinitialize is set to a non-empty list of subdomains, otherwise no "
    ...
    const std::unordered_map<dof_id_type, std::pair<SubdomainID, SubdomainID>> & moved_elems)
{
  ...
  for (const auto & [elem_id, subdomain] : moved_elems)
  {
    ...
    {
      const auto & [from, to] = subdomain;
      if (subdomainIsReinitialized(to) && _original_subdomain_in_list)
        _reinitialized_elems.insert(elem_id);
      // Only reinitialize if original subdomain is not in list of subdomains
      else if (subdomainIsReinitialized(to) && !_original_subdomain_in_list &&

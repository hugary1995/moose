...
EqualValueBoundaryConstraint::validParams()
{
  ...
  params.addParam<BoundaryName>(
      "secondary", "", "The boundary ID associated with the secondary side");
...
EqualValueBoundaryConstraint::EqualValueBoundaryConstraint(const InputParameters & parameters)
  : NodalConstraint(parameters),
    _primary_node_id(getParam<dof_id_type>("primary")),
...
EqualValueBoundaryConstraint::updateConstrainedNodes()
{
  ...
  if ((_secondary_node_ids.size() == 0) && (_secondary_node_set_id == ""))
    mooseError("Please specify secondary node ids or boundary id.");
  else if ((_secondary_node_ids.size() == 0) && (_secondary_node_set_id != ""))
  {
    ...
    if (_primary_node_id == std::numeric_limits<dof_id_type>::max())
    {
  ...
  // const auto node_to_elem_pair = node_to_elem_map.find(_primary_node_vector[0]);
  ...
  //           const dof_id_type n_nodes = elem->n_nodes();
...
{
  const auto & dofmap = _sys.dofMap();
  const auto vn = _var.number();
  std::vector<dof_id_type> di;
  dofmap.dof_indices(_mesh.nodePtr(_primary_node_vector[0]), di, vn);
  std::cout << "primary node: " << _primary_node_vector[0] << "(" << di[0] << ")" << std::endl;
  std::cout << "secondary nodes:";
  for (const auto nid : _connected_nodes)
  {
    dofmap.dof_indices(_mesh.nodePtr(nid), di, vn);
    std::cout << " " << nid << "(" << di[0] << ")";
  }
  std::cout << std::endl;
  ...
  std::cout << "primary dof: " << primarydof[0] << std::endl;
  std::cout << "secondary dofs:";
  for (const auto dof : secondarydof)
    std::cout << " " << dof;
  std::cout << std::endl;
